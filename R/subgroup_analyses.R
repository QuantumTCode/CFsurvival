...
subgroup.CFsurvfit <- function(fit, subgroup.inds, conf.band=TRUE, conf.level=.95, surv.diffs=TRUE, surv.ratios=TRUE, risk.ratios=FALSE, nnt=FALSE, verbose=FALSE) {
    ...
    nuis <- fit$nuisance
    g.hats <- fit$nuisance$prop.pred[subgroup.inds]
    ...
    if(1 %in% fit.treat) {
        ...
        S.hats.1 <-fit$nuisance$event.pred.1[subgroup.inds,]
        G.hats.1 <-fit$nuisance$cens.pred.1[subgroup.inds,]
        surv.1 <- .get.survival(Y=time, Delta=event, A=treat, fit.times=fit.times, eval.times=nuis$eval.times, S.hats=S.hats.1, G.hats=S.hats.1, g.hats=g.hats)
    ...
    if(0 %in% fit.treat) {
        ...
        S.hats.0 <-fit$nuisance$event.pred.0[subgroup.inds,]
        G.hats.0 <-fit$nuisance$cens.pred.0[subgroup.inds,]
        surv.0 <- .get.survival(Y=time, Delta=event, A=1-treat, fit.times=fit.times, eval.times=nuis$eval.times, S.hats=S.hats.0, G.hats=G.hats.0, g.hats=1-g.hats)
